...
for($i=1;$i<=$sum_users;$i++){
    try{
    	...
    	//目標値取得
        $stmt=$db->query("SELECT mokuhyou_total FROM demomokuhyouti WHERE id = {$i}");
     ...
     try{
        ...
        $db->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
    		//実測値ぽいんと取得
    		$stmt=$db->query("SELECT total FROM demo_mokuhyoutasseiritu WHERE id = {$i}");//user_IDの部分変数に置き換える
  ...
  <div class="search"> 
    ...
    <div class=main> 
      ...
      <script>
		    ...
		    target=JSON.parse('<?php echo $target_json;?>');
		    ...
		    percent=JSON.parse('<?php echo $percent_json; ?>');
